...
def _session_path(devterminal_dir: Path) -> Path:
    return devterminal_dir / SESSION_FILENAME


def save_worktree_session(
    devterminal_dir: Path,
    session: WorktreeSession | None,
) -> None:
    path = _session_path(devterminal_dir)
...
def load_worktree_session(devterminal_dir: Path) -> WorktreeSession | None:
    path = _session_path(devterminal_dir)
    if not path.exists():
        return None
    try:
