...
from devterminal.worktree.setup import perform_post_creation_setup
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".devterminal" / "worktrees"
        )
        self._devterminal_dir = Path(repo_root) / ".devterminal"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._devterminal_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._devterminal_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._devterminal_dir)
        if session is None:
            return None
