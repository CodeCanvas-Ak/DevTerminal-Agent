...
from devterminal.context import (
    CompactBoundary,
...
from devterminal.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from devterminal.conversation import ThinkingBlock as ConvThinkingBlock
from devterminal.memory.auto_memory import MemoryManager
from devterminal.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from devterminal.hooks import HookContext, HookEngine, ToolRejectedError
from devterminal.hooks.engine import HookNotification
from devterminal.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from devterminal.tools import ToolRegistry
from devterminal.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".devterminal" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".devterminal" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from devterminal.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from devterminal.context.manager import (
