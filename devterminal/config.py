...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".devterminal" / "config.yaml",
        cwd / ".devterminal" / "config.yaml",
        cwd / ".devterminal" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .devterminal/config.yaml "
            "in project or ~/.devterminal/config.yaml"
        )
    return merged
