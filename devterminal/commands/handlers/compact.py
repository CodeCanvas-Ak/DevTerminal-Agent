...
from devterminal.commands.registry import Command, CommandContext, CommandType
...
async def handle_compact(ctx: CommandContext) -> None:
    ...
    from devterminal.agent import CompactNotification, ErrorEvent
    ...
    if isinstance(result, CompactNotification):
        ...
        if ctx.session is not None and result.boundary is not None:
            from devterminal.memory.session import make_compact_boundary
