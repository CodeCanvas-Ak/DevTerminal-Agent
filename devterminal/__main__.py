...
from devterminal.config import ConfigError, load_config
from devterminal.hooks import HookConfigError, HookEngine, load_hooks
from devterminal.permissions import PermissionMode


def main() -> None:
    # 先确保 .devterminal/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".devterminal").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".devterminal/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="devterminal", description="DevTerminal Agent AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    from devterminal.app import DevTerminal AgentApp
    from devterminal.driver import NoAltScreenDriver

    app = DevTerminal AgentApp(
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str) -> None:
    from devterminal.agent import Agent
    from devterminal.client import create_client, resolve_context_window
    from devterminal.conversation import ConversationManager
    from devterminal.memory.instructions import load_instructions
    ...
    from devterminal.tools import create_default_registry
