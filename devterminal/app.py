...
from devterminal.client import (
    AuthenticationError,
    LLMClient,
    LLMError,
    create_client,
...
from devterminal.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from devterminal.commands.completion import CompletionPopup
from devterminal.commands.handlers import register_all_commands
from devterminal.config import MCPServerConfig, ProviderConfig
from devterminal.hooks import HookContext, HookEngine, load_hooks
from devterminal.conversation import ConversationManager, Message
from devterminal.mcp import MCPManager
from devterminal.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from devterminal.permissions import (
...
from devterminal.agents.task_manager import TaskManager
from devterminal.agents.trace import TraceManager
from devterminal.agents.notification import inject_task_notifications
from devterminal.commands.handlers.tasks import create_tasks_command
from devterminal.skills.executor import SkillExecutor
from devterminal.skills.loader import SkillLoader
from devterminal.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from devterminal.cache import FileCache
from devterminal.tools import ToolRegistry, create_default_registry
from devterminal.tools.agent_tool import AgentTool
from devterminal.tools.ask_user import AskUserEvent, AskUserTool
from devterminal.tools.impl.tool_search import ToolSearchTool
from devterminal.tools.load_skill import LoadSkill
from devterminal.worktree.cleanup import start_stale_cleanup_task
from devterminal.worktree.manager import WorktreeManager
from devterminal.commands.handlers.worktree import create_worktree_command
from devterminal.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".devterminal", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".devterminal" / "history"
...
class DevTerminal AgentApp(App):
    CSS_PATH = "styles.tcss"
    TITLE = "DevTerminal Agent"
    INLINE_PADDING = 0
    theme = "devterminal"
    BINDINGS = [
    ...
    def _make_banner(model: str = "", work_dir: str = "") -> RichText:
        ...
        t.append("DevTerminal Agent v0.1.0\n", style="color(242)")
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".devterminal" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".devterminal" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".devterminal" / "permissions.local.yaml",
            ),
        ...
        from devterminal.filehistory import FileHistory
        ...
        from devterminal.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from devterminal.config import WorktreeConfig
        ...
        from devterminal.tools.enter_worktree import EnterWorktreeTool
        from devterminal.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from devterminal.teams.manager import TeamManager
        from devterminal.tools.team_create import TeamCreateTool
        from devterminal.tools.team_delete import TeamDeleteTool
        ...
        from devterminal.commands.handlers.trace import create_trace_command
        ...
        from devterminal.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from devterminal.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from devterminal.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from devterminal.plan_dialog import InlinePlanWidget, PlanChoice
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from devterminal.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from devterminal.permission_dialog import InlinePermissionWidget
