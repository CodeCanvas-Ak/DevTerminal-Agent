...
from devterminal.teams.backend_detect import BackendDetectionError, detect_backend
from devterminal.teams.coordinator import (
    get_coordinator_system_prompt,
    get_coordinator_user_context,
    is_coordinator_mode,
    match_session_mode,
)
from devterminal.agents.tool_filter import (
...
from devterminal.tools import ToolRegistry
from devterminal.tools.base import Tool, ToolResult
...
class TestModels:
    ...
    def test_unique_team_name(self, tmp_dir):
        with patch("devterminal.teams.models.Path.home", return_value=Path(tmp_dir)):
            name1 = unique_team_name("my-team")
            assert name1 == "my-team"
            (Path(tmp_dir) / ".devterminal" / "teams" / "my-team").mkdir(parents=True)
...
class TestBackendDetect:
    ...
    def test_iterm2_with_it2(self):
        ...
        with patch.dict(os.environ, env, clear=False):
            with patch("devterminal.teams.backend_detect.shutil.which") as mock_which:
    ...
    def test_no_backend_raises(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TMUX", None)
            os.environ.pop("TERM_PROGRAM", None)
            with patch("devterminal.teams.backend_detect.shutil.which", return_value=None):
...
class TestConfigExtensions:
    def test_teammate_mode_defaults(self):
        from devterminal.config import AppConfig
    ...
    def test_load_config_with_team_fields(self, tmp_dir):
        from devterminal.config import load_config
...
class TestTranscript:
    ...
    def test_save_and_load(self, tmp_dir):
        ...
        from devterminal.teams.transcript import load_transcript, save_transcript
    ...
    def test_load_nonexistent(self, tmp_dir):
        from devterminal.teams.transcript import load_transcript
        with patch("devterminal.teams.models.Path.home", return_value=Path(tmp_dir)):
...
class TestAgentCoordinatorIntegration:
    def test_normal_prompt(self):
        from devterminal.prompts import build_system_prompt, BASE_PERSONA
        prompt = build_system_prompt()
        assert BASE_PERSONA in prompt

    def test_coordinator_prompt(self):
        from devterminal.prompts import build_system_prompt
    ...
    def test_coordinator_overrides_plan(self):
        from devterminal.prompts import build_system_prompt, PLAN_MODE_INSTRUCTIONS
