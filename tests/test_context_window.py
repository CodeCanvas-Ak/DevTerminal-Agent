...
from devterminal.client import resolve_context_window
from devterminal.config import ProviderConfig
from devterminal.validator import (
    ConfigError,
...
class TestAutoFetch:
    ...
    async def test_fetch_success_is_cached_and_used(self):
        ...
        with patch("devterminal.client.create_client", return_value=fake) as mk:
    ...
    async def test_fetch_raises_degrades_to_mapping_table(self):
        ...
        with patch("devterminal.client.create_client", return_value=fake):
            # 不应抛出异常。
            await resolve_context_window(p)
        # 对 claude 回退到映射表。
    ...
    async def test_client_construction_failure_degrades(self):
        ...
        p = _provider(model="claude-sonnet-4-6")
        with patch(
            "devterminal.client.create_client",
    ...
    async def test_non_anthropic_provider_is_not_fetched(self):
        ...
        with patch("devterminal.client.create_client") as mk:
            await resolve_context_window(p)
