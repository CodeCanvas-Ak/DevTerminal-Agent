...
class TestCommandExecutor:
    ...
    async def test_normal_execution(self):
        from devterminal.hooks.executors import execute_command
...
class TestPromptExecutor:
    ...
    async def test_returns_message(self):
        from devterminal.hooks.executors import execute_prompt
...
class TestHttpExecutor:
    ...
    async def test_mock_request(self):
        from devterminal.hooks.executors import execute_http

        action = Action(type="http", url="https://httpbin.org/post", body='{"test": true}')
        ctx = HookContext()
        # 用 mock 避免发起真实的网络请求
        with patch("devterminal.hooks.executors.urlopen") as mock_urlopen:
...
class TestAgentExecutor:
    ...
    async def test_stub(self):
        from devterminal.hooks.executors import execute_agent
...
class TestExecuteAction:
    ...
    async def test_dispatch(self):
        from devterminal.hooks.executors import execute_action
...
class TestAgentHookIntegration:
    ...
    async def test_pre_tool_use_reject_skips_tool(self):
        from devterminal.agent import Agent, ToolResultEvent
        ...
        from devterminal.tools.base import StreamEnd, StreamEvent, TextDelta, ToolCallComplete
