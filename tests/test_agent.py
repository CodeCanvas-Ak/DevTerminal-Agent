...
from devterminal.serialization import build_anthropic_messages
...
async def test_multi_step_autonomous():
    ...
    client = MockLLMClient([
        ...
        [
            ...
            ToolCallComplete("t1", "WriteFile", {"file_path": "/tmp/devterminal_test_hello.txt", "content": "Hello World"}),
            StreamEnd("end_turn", input_tokens=10, output_tokens=20),
        ],
        # 第 2 轮：ReadFile 进行验证
        [
            TextDelta("Verifying content."),
            ToolCallComplete("t2", "ReadFile", {"file_path": "/tmp/devterminal_test_hello.txt"}),
...
def test_partition_tool_calls():
    """分批逻辑会把可并发执行的调用归到同一组。"""
    from devterminal.tools.base import ToolCallComplete
...
def test_system_prompt_normal():
    ...
    assert "DevTerminal Agent" in sp
