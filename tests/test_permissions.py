...
class TestPathSandbox:
    ...
    def test_temp_dir_allowed(self) -> None:
        tmp = Path(tempfile.gettempdir()) / "devterminal_test.txt"
...
class TestRuleEngine:
    ...
    def test_append_local_rule(self) -> None:
        tmpdir = Path(tempfile.mkdtemp())
        local_path = tmpdir / ".devterminal" / "permissions.local.yaml"
...
class TestPermissionChecker:
    ...
    def test_dangerous_command_denied(self) -> None:
        from devterminal.tools.bash import Bash
        tool = Bash()
    ...
    def test_path_outside_sandbox_denied(self) -> None:
        from devterminal.tools.read_file import ReadFile
        tool = ReadFile()
    ...
    def test_write_tool_asks_in_default_mode(self) -> None:
        from devterminal.tools.write_file import WriteFile
        tool = WriteFile()
...
async def test_e2e_rule_allows_git():
    ...
    tmpdir = Path(tempfile.mkdtemp())
    rules_file = tmpdir / ".devterminal" / "permissions.yaml"
