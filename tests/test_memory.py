...
from devterminal.conversation import (
    ConversationManager,
    Message,
    ToolResultBlock,
    ToolUseBlock,
...
from devterminal.memory.instructions import (
    MAX_INCLUDE_DEPTH,
    load_instructions,
    process_includes,
)
from devterminal.memory.session import (
    RecordType,
    ResumeResult,
    Session,
    SessionManager,
...
class TestLoadInstructions:
    def test_single_layer(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        devterminal_md = tmp_path / "MEWCODE.md"
        devterminal_md.write_text("project instructions", encoding="utf-8")
...
class TestSession:
    def test_append_writes_jsonl_and_updates_meta(self, tmp_path: Path) -> None:
        sessions_dir = tmp_path / ".devterminal" / "sessions"
...
class TestMemoryManager:
    ...
    def test_load_merges_user_and_project(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        user_mem = fake_home / ".devterminal" / "memories.md"
        user_mem.parent.mkdir(parents=True)
        user_mem.write_text("### 用户偏好\n- prefer spaces", encoding="utf-8")

        project_mem = tmp_path / "project" / ".devterminal" / "memories.md"
...
class TestConversationInjection:
    def test_inject_long_term_memory(self) -> None:
        ...
        assert "devterminalMd" in conv.history[1].content
    ...
    def test_inject_instructions_only(self) -> None:
        ...
        assert "devterminalMd" in conv.history[0].content
...
class TestMemoryExtraction:
    def test_extraction_prompt_contains_categories(self, tmp_path: Path) -> None:
        from devterminal.memory.auto_memory import MEMORY_EXTRACTION_PROMPT
